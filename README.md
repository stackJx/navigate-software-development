...
| [开发思路](references/3-development-planning.md) | **必选，开发前输出** | 现状映射、纵向切片、依赖关系和可执行计划 |
...
- 每次都在首个变更动作前，以固定顺序输出 `## 需求分析`、`## 开发思路`、`## 验收标准`。
...
- 不确定三个必选产物该写多长、长什么样时，对照 [走查示例](references/5-worked-example.md)（含跳过路径与全命中路径两种形态）。
- 复杂任务可按门禁把独立、无共享状态的子任务委派给 Codex 子智能体并行推进，再扇入合并回三个必选章节（默认 solo，开启 `multi_agent` 后生效；详见 SKILL.md「子智能体委派」节）。
